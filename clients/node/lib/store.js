'use strict';
// Writes verified payloads out.
//
// NO CONVERSION HAPPENS HERE. The camera emits JPEG and the chunker moves
// opaque bytes, so this is a write. If this file ever grows an image library,
// something has gone wrong upstream.

const fs = require('fs');
const path = require('path');

const EXT = { 1: 'json', 2: 'jpg', 3: 'log' }; // PT_SCHEMA / PT_IMAGE / PT_LOG
...
class PayloadStore {
  ...
  // A transfer may span interruptions (the radio is busy, or an outside
  // influence cuts it off), so partial progress is persisted and continued.
  // Keyed by node+pid; the sidecar carries crc/count/len so a resume is
  // REJECTED if the payload behind that pid changed (a 16-bit pid collision
  // between two distinct images) rather than blending two images.
  // See specs/chunk-resume.md.
  _partPaths(node, pid) {
    const sub = path.join(this.dir, String(node).replace(/[^\w!-]/g, '_'));
  ...
  savePartial(node, { pid, crc, count, len, have, buf }) {
    ...
    // Buffer first, then the sidecar: a crash between the two leaves a buffer
    // with no sidecar, which loadPartial treats as "no partial" — safe. The
    // reverse (sidecar without buffer) would claim progress we cannot back up.
  ...
  // images accumulate until something decides otherwise, and that policy has
  // not been agreed. See SPEC.md open question 2.
  prune() { throw new Error('retention policy not implemented — see SPEC.md §7'); }
}
