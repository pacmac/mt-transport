...
const REPLY_DEADLINE = 25000, CHUNK_DEADLINE = 180000;
...
const TYPES = [
  ...
  { name: 'config', text: t => `@${t} config`, ev: 'payload', ok: o => o && o.type === 'config' },
...
(async () => {
  ...
  const push = (type, hop, ms) => { (rec[type] ??= {}); (rec[type][hop] ??= []).push(ms); };
  ...
  for (let rep = 0; rep < REPS; rep++) {
    for (const hop of HOPS) {           // interleaved
      ...
      for (const ty of TYPES) {
        ...
        const obj = await p;
        push(ty.name, hop, obj ? Date.now() - t0 : null);
      ...
      if (DO_CHUNK) {
        ...
        push('chunk', hop, ok ? Date.now() - t0 : null);
  ...
  console.log('\n         ' + HOPS.map(h => `hop${h}`.padStart(11)).join(''));
  for (const nm of names) {
    ...
    for (const hop of HOPS) {
      ...
      const cell = good.length
        ? `${median(good).toFixed(1)}s ${loss}/${xs.length}L`
        : `-- ${loss}/${xs.length}L`;
      line += cell.padStart(11);
    }
    console.log(line);
  }
  console.log('\n(cell = median round-trip · losses/attempts)');
  process.exit(0);
