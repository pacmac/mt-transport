...
// Apply the RFC 7748 clamp (dh1 does this to its generated key). Idempotent, so it is
// safe to apply to a key from any source. We clamp ONCE on storage so key derivation
// and ECDH can never disagree about which scalar they are using.
void pkiClampPrivate(uint8_t priv[32]);

// Derive the public key from a private key (eval against the base point). Deriving it
// on-device rather than storing a matching pair removes a whole class of error: a
// public key generated by some other tool with different clamping can never disagree
// with what this device actually encrypts under.
